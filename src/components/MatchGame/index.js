...
class MatchGame extends Component {
  ...
  onclickPlayAgainBtn = () => {
    this.setState({isGameRunning: true, score: 0, seconds: 60})
  }
  ...
  clickImage = id => {
    const {seconds, clickImageId} = this.state
    const res = Math.floor(Math.random() * imagesList.length)
    if (seconds !== 0 && clickImageId === id) {
      this.setState(prevState => ({
        imageUrl: imagesList[res].imageUrl,
        score: prevState.score + 1,
        clickImageId: imagesList[res].id,
      }))
    } else {
      clearInterval(this.timerId)
  ...
  render() {
    const {activeTabId, imageUrl, seconds, score, isGameRunning} = this.state
    const resultantSeconds = seconds > 9 ? seconds : `0${seconds}`
        ...
        <ul className="navbar-container">
          ...
          <li className="item">
            <p className="paragraph">
              Score: <span className="count-number">{score}</span>
            </p>
          </li>
          <li className="item">
            <img
          ...
          <li className="item">
            <p className="counter">{resultantSeconds} sec</p>
          </li>
        </ul>
        {isGameRunning ? (
